...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (newsletterBtn && newsletterInput) {
    newsletterBtn.addEventListener('click', function () {
      ...
      newsletterInput.style.borderColor = 'rgba(61, 107, 53, 0.45)';
      newsletterBtn.textContent = '✓';
      newsletterBtn.style.background = '#3d6b35';
  ...
  const animTargets = document.querySelectorAll(
    '.herb-card, .review-card, .story-value-card'
  );
  ...
  if (mainImg) {
    mainImg.addEventListener('mousemove', function (e) {
      ...
      mainImg.style.setProperty('--x', x + '%');
      mainImg.style.setProperty('--y', y + '%');
    });
  }
